...
#else
            mr_trace_bump_range((lispobj*)a->base, (lispobj*)a->freeptr);
#endif
            chain = a->link;
        } while (chain != NIL);
    }
...
#ifdef LISP_FEATURE_MARK_REGION_GC
/* Broke fullcgc by not having cons mark bits - sorry about that. */
int find_dynspace_to_arena_ptrs(__attribute__((unused)) lispobj result_buffer) {
  fprintf(stderr, "WARNING: No dynspace -> arena checker with :MARK-REGION-GC.\n");
  return 0;
}
#else
